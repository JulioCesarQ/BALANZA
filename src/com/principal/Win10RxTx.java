...
//worker threads to be removed when the COM port is closed
	...
	/**
	 * @author liy Worker thread to read the port continuously and add the result to
	 *         the list. Can be interrupted externally
	 */
						...
						if ((currentByte != NUL) && (currentByte != LF)) {
							...
							if (currentByte == CR) {
								if (dataIndex != 0) {
									...
									addToList(new String(data, 0, dataIndex, StandardCharsets.UTF_8));
	...
	/**
	 * @author liy Worker thread to write data to the port until finished Can be
